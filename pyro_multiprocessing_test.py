...
def synthetic_data_single_subject(model = 'ddm',
    ...
    for i in range(n_parameter_sets):
        out_dict[i] = {}
        out_dict[i]['gt_params'] = {key_: parameter_samples_dict[key_][i] for key_ in parameters_samples_dict.keys()}
        data_tmp = sim_wrap(theta = np.array([out_dict[i]['gt_params'][key_] for key_ in out_dict[i]['gt_params'].keys()]).astype(np.float32).tile((n_samples,1)),
                            model = model,
                            out_type = 'numpy')
        
        out_dict[i]['data_pyro'] = torch.tensor(data_tmp)
        
        # preprocess data to be hddm ready
        data_tmp_pd = pd.DataFrame(data_tmp, columns = ['rt', 'reponse'])
        data_tmp_pd['subj_idx'] = 0
        
        out_dict[i]['data_hddm'] = data_tmp_pd
...
def synthetic_data_hierarchical(model = 'ddm',
    ...
    for i in range(n_parameter_sets):
        ...
        for param in model_config['params']:
            ...
            param_mu = np.random.uniform(low = min_adj, 
                                         high = max_adj)
            ...
            param_dict[i][param + '_mu'] = param_mu.astype(np.float32)
            param_dict[i][param + '_std'] =  param_std.astype(np.float32)
    ...
    for i in range(n_parameter_sets):
        out_dict[i] = {}
        out_dict['gt_params'] = deepcopy(param_dict[i])
        
        theta_tmp = np.repeat(np.stack([param_dict[i][param + '_mu'] for param in model_config['params']]).T, n_subjects, axis = 0)
        ...
        # pyro data
        data_pyro = torch.tensor(np.resize(out, (n_subjects, n_samples_by_subject, 2)).swapaxes(0, 1).astype(np.float32))
        out_dict['data_pyro'] = data_pyro
